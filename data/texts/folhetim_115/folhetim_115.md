...
<sup>&</sup>lt;sup>1</sup> A geometria sintética elementar, no tratamento de congruência de figuras geométricas, vê medidas de ângulos e de segmentos como propriedades intrínsecas ou como elementos de axiomatização. É interessante ver a mudança de atitude ao longo dos tomos dos *Elementos* de Euclides. Segundo Cajori (*History of Mathematics*, Chelsea, Fourth Ed., 1985), a geometria grega até (inclusive) Euclides, não era muito versada em teoremas sobre medição de objetos geométricos. Aparentemente, foram Arquimedes e Apolônio os grandes pioneiros da geometria métrica propriamente dita.
...
Mas, o que eram exatamente *relações* e *simetrias*? Lagrange jamais introduziu estas noções explicitamente. Em verdade, é mesmo muito difícil em meio à longa e inextricável sequência de relações e igualdades produzidas no artigo, divisar o método a ele
...
Um polinômio  $\mathbf{F}$  é dito ser *invariante* por uma permutação  $\sigma$  dos índices  $\{1,...,n\}$  se  $F(x_{\sigma(1)},...,x_{\sigma(n)})$ . Uma simetria de f(x). O conjunto das simetrias de f(x) constitui um subgrupo simétrico de grau n
